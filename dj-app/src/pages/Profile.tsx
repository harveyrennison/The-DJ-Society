    ...
    <Box sx={{ minHeight: '100vh', pb: 8 }}>
      ...
      <Box sx={{ height: 350, width: '100%', overflow: 'hidden', position: 'relative' }}>
        ...
        <Button 
            ...
            onClick={onBack}
            sx={{ position: 'absolute', top: 100, left: 20, bgcolor: 'rgba(0,0,0,0.5)', '&:hover': { bgcolor: 'rgba(0,0,0,0.8)' } }}
        >
            {isOwner ? 'Edit Profile' : 'Back to Directory'}
        </Button>
      </Box>
        ...
        <Grid container spacing={4}>
          ...
          <Grid sx={{ pt: { xs: '12px', md: '4px' } }}>
            <Card sx={{ p: 3, textAlign: 'center' }}>
              ...
              <Button fullWidth variant="contained" size="large" sx={{ mb: 2 }}>Book Now</Button>
              <Button fullWidth variant="outlined" color="secondary">Follow</Button>
            </Card>
          </Grid>
              ...
              {/* <Stack spacing={2}>
                {tracks.map((track, index) => (
                  <Paper key={index} sx={{ p: 2, display: 'flex', alignItems: 'center', bgcolor: 'rgba(255,255,255,0.03)', '&:hover': { bgcolor: 'rgba(255,255,255,0.06)' } }}>
                    ...
                    <IconButton color="primary" sx={{ mr: 2 }}><PlayArrow /></IconButton>
                    <Box sx={{ flexGrow: 1 }}>
                      <Typography variant="subtitle1">{track.title}</Typography>
                      <Typography variant="caption" color="text.secondary">{track.plays} plays</Typography>
                    </Box>
                    <Typography variant="body2" color="text.secondary">{track.duration}</Typography>
                  </Paper>
                ))}
            ...
            {/* <Box>
              ...
              <Grid container spacing={2}>
                {upcoming.map((gig, index) => (
                  <Grid sx={{ pt: { xs: '12px', md: '6px' } }} key={index}>
                    <Card variant="outlined" sx={{ p: 2, borderColor: 'rgba(245, 0, 87, 0.3)' }}>
                        <Stack direction="row" alignItems="center" spacing={2}>
                            <Box sx={{ textAlign: 'center', p: 1, bgcolor: 'rgba(245, 0, 87, 0.1)', borderRadius: 1, minWidth: 60 }}>
                                <Typography variant="h6" color="secondary" sx={{ lineHeight: 1 }}>{gig.date.split(' ')[1]}</Typography>
                                <Typography variant="caption" sx={{ textTransform: 'uppercase' }}>{gig.date.split(' ')[0]}</Typography>
                            </Box>
                            <Box>
                                <Typography variant="subtitle1" fontWeight="bold">{gig.venue}</Typography>
                                <Typography variant="body2" color="text.secondary">{gig.city}</Typography>
                            </Box>
                        </Stack>
                    </Card>
                  </Grid>
